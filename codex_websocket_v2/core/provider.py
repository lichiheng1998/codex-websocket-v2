...
async def list_models_for_async(
    ...
    *,
    ...
    """Async version of ``list_models_for`` for use on the bridge loop.

    Prefers the direct HTTP path when a provider base_url is configured
    (same as the sync version), then pages through ``model/list`` via RPC.
    """
    if provider.has_base_url():
        direct = fetch_provider_models_http(provider.base_url, provider.env_key)
        if direct["ok"]:
